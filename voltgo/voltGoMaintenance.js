...
function isMaintenanceDue(station) {
    ...
    const lastMaintenance =
        new Date(station.lastMaintenance);
...
function getValidLicenceCountry(country) {
    ...
    return licencePlateFormats.find(
        licenceCountry =>
            normalizeValue(licenceCountry.country) ===
...
function validatePhone(
    ...
    const phoneCountry =
        countryPrefixes.find(
            country =>
                country.prefix === phonePrefix
        );
...
function validateDob(dob) {
    ...
    if (
        isNaN(date.getTime()) ||
        date.getFullYear() !==
        Number(dob.substring(0, 4)) ||
        date.getMonth() + 1 !==
        Number(dob.substring(5, 7)) ||
        date.getDate() !==
        Number(dob.substring(8, 10))
    ) {

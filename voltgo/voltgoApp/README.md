...
Funcionamento: O sistema valida o intervalo temporal ('startDate' e 'endDate') antes de fazer o registo de uma nova sessão. Caso exista sobreposição com outro carregamento ativo no mesmo posto, a operação é interrompida e é emitida uma mensagem de erro.
...
2.2 Gestão da disponibilidade técnica das estações
...
2.4.  Sistema de Alerta de Manutenção Preventiva (Dashboard)



Funcionamento: Na inicialização da dashboard, o sistema calcula a diferença em dias entre a data atual e a data da última manutenção registada para cada posto. Se o valor for igual ou superior a 15 dias, é gerado um aviso em texto no terminal a dizer que é recomendado fazer manutenção no posto.
...
Funcionamento: Os consumos energéticos e o valor a faturar são calculados com base no tempo de utilização, na potência usada (standard ou rápida) e no tarifário associado.
...
Funcionamento: A criação de uma sessão paga de carregamento resulta no cálculo e crédito automático de pontos de fidelidade no registo do cliente correspondente.

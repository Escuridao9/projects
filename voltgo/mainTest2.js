...
// Generates an ID using BOTH active and inactive lists.
// This means an ID will never be reused.
...
function showStations() {
    ...
    console.log(
        "\nID | Code | District | Power | Connector | Status"
    );
    ...
    for (const station of stations) {
        ...
        console.log(
            `${station.id} | ${station.code} | ${station.district} | ${station.power} kW | ${station.connectorType} | ${station.status}`
        );
    ...
    do {
        ...
        switch (option) {
            ...
            case "4":
                ...
                const removeTif = input("TIF: ");

                removeClient(removeTif);
...
function showCharges() {
    ...
    for (const charge of charges) {
        ...
        console.log(
            `${charge.id} | ${charge.stationCode} | ${charge.clientId} | ${charge.startDate} | ${charge.endDate} | ${charge.duration} | ${charge.energy} kWh | ${charge.dataPlanId} | ${charge.cost} € | ${charge.status}`
        );
...
function createCharge(
    ...
    // Only calculate after validation succeeds
    const energy =
        calculateEnergy(
            stationCode,
            startDate,
